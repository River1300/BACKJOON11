...
알고리즘 기초
void qsort()
주어진 포인터에서 원소 개수만큼 컨테이너에 qsort알고리즘을 적용한다. 정렬을 위한 값 비교는
함수 포인터로 받아서 처리한다.
벡터에 qsort를 내림차순으로 적용하고, 화면에 출력하는 프로그램이다. 매우 짧지 안나
각 매개변수들은 위에서 언급한 함수 원형을 참고하면 된다.
비교 함수ㅡㄴ 매우 간단하 것

람다식
코드내에 함수를 선언없이 삽입하는 기법을 말한다. 자세한 내용ㅇ느 다음에 다루기로 하고 형태만 살펴보만

for each
범위 기반 반복문과 비슷한 기능을 수행하는 것이 알고리즘에 있다.
비슷한 기능이지만 foreach선언세어 보이는 것 처럼 시작 끝 지점을 지정해 줄 수 있다는 장점이 있다.
그리고 템플릿에서 보면 함수가 class로 되어 있다는 것도 보인다. 이를 Functionㅒㅠㅓㄷㅊㅅfk gksek.
클래스를 함수처럼 작동하게 만들 수 있어서 for each는 더욱 더 유연한 코딩이 가능하다.

STL 컨테이너
순차 컨테이너 array vector forward_list list deque
보조 컨테이너 stack queue priority_queue
연관 컨테이너 set multiset map mulimap
순차 컨테이너 원소들이 순서대로 나열되어 있는 컨테이너이다. 원소를 추가해도 순서를 유지하게 된다.
보조 컨테이너 특정한 목적에 맞도록 변형된 컨테이더
연관 컨테이너 원소들이 좌우 혹은 상하로 서로 관련이 있는 컨테이너이다. 원소들끼리 부모자식의 관계를
가지고 있기도 한다.
원소를 추가하면 미리 정의된 순서에 따라 정렬이 된다.
오버헤드
간접 비용이라고 해석할 수 있다. 기대하는 저장공간 외에 추가되는 값을 말한다.
알고리즘을 평가할 때 공간 복잡도를 기억할것
컨테이너 자체에 대한 공간 복잡도라고 이해해도 된다.
정수 원소 4개를 가지고 있는 컨테이너는 4*4가 기대치 이지만 내부적으로 처리하기 위해 추가로
사용되는 메모리가 발생할 수 있는데 이를 오버헤드라 한다.

메모리 접근 방식
임의 접근 직접 접근이라고도 부르며 원소가 포함된 메모리를 상수 시간에 가져올 수 있다.
순차 접근 처음 혹은 끝 부터 순서대로 접근해서 원소를 가져온다.

std::array
array는 고정 크기의 순차 컨테이너이다. 구조적으로 보면 일반 배열과 동일하다. 단, STL의 반복자나
알고리즘이 배열을 사용할 수 있도록 기능만을 추가한 것
순차적 정해진 순서로 저장되며, 각 우너소는 이 순서대로 접근할 수 있다.
연속된 저장 공간 연속된 메모리에 저장되어 포인터로 이동이 가능하다. 랜덤 액세스가 가능하다.
고정 크기 array의 크기는 컴파일 시간 상수이다. 일반 배열과 마찬가지로 정확히 원소 개수만큼 메모리를 차지하며
할당에 시간이 필요하지 않다.
std::vector
vector는 동적으로 크기를 변경할 수 있는 배열을 나타내는 순차 컨테이너이다.
순차적 정해진 순서로 저장되며, 각 우너소는 이 순서대로 접근할 수 있다.
동적 배열 배열 크기를 변경할 수 있으며, 가장 마지막에 추가제거할 때 가장 효율적이다. 포인터 연산을
통해 바로 접근할 수 있다.
메모리 할당자
할당자를 통해 추가할 우너소의 메모리 할당을 동적으로 처리할 수 있다.
std::forward_list
단일 연결 리스트로 임의의 위치에 추가 삭제가 가능한 순차 컨테이너이다. 
순차적 정해진 순서로 저장되며, 각 우너소는 이 순서대로 접근할 수 있다.
단일 연결 리스트 특정 우너소 이후에 상수 시간에 삽입 삭제가 가능하지만 임의 접근은 가능하지 않다.
메모리 할당자 할당자를 통해 추가할 원소의 메모리 할당을 관리할 수있다.
연결 리스트 컨테이너는 정렬 멤버 함수를 제동한다. qsort()는 일반화된 버전이라 void*로 처리하느라
Compare 객체가 필요하지만 list.sort()는 참과 거짓만을 반환하는 함수 객체가 필요하다.
참이면 첫번째 비교 원소가 앞으로 오고 거시이면 되로 간다.

이렇게 bool형을 반환하는 함수 객체를 간단히 서술자라 부른다.

std::list
이중 연결 리스트로 임의의 위치에 추가삭제가 가능한 순차 컨테이너이다.
순차적 정해진 순서로 저장되며, 각 우너소는 이 순서대로 접근할 수 있다.
이중 연결 리스트 특정 원소 이후에 상수 시간에 삽입 삭제가 가능하지만, 임의 접근은 불가능 하다.
메모리 할당자 할당자를 통해 추가할 원소의 메모리 할당을 관리할 수 있다.

std::deque
덱은 double ended queue의 약자로 양방향 큐이다. 앞 혹은 뒤에ㅓ 추가하거나
삭제할 수 있는 순차적 컨테이너 이다.
순차적
정해진 순서대로 저장되며 각 우너소는 이 순서대로 접근하 수 있다.
동적 배열
배열 크기를 변경할 수 있으며, 가장 마지막에 추가제거 할 때 가장 효율적이다. 포인터 연산을 통해 바로
접근할 수있다.
메모리 할당자
할당자를 통해 추가할 원소의 메모리 할당을 관리할 수 있다.

vector와 deque는 기능 적으로 매우 유사하지만 내부적으로는 상당히 다른 방식으로 작동한다.
vector는 원소의 내부 관리를 위해 단일 배열을 사용하고 해당 배열을 재할당 하지만, deque는 서로 다른 메모리
공간으로 분산될 수있다.
